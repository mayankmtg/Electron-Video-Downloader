...
//shell.config.execPath('./curl.exe');
//shell.common.config.execPath('./curl.exe');
...
function Download(num){
	...
	console.log("Merging");
	if (shell.exec('head -c -1 -q '+name+'_*.jpg > '+name+'.jpg').code !== 0) {
		shell.echo('Error');
		shell.exit(1);
	}
	else{
	...
	if (shell.exec('rm '+name+'_* ').code !== 0) {
		shell.echo('Error');
		shell.exit(1);
	}
	else{
